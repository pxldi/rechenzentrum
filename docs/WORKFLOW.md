...
workloads and PVC bindings. Follow `MIGRATION.md` for source/ownership changes.
Fix or safely revert regressions within the authorized task.
...
notifications, image publication and scheduled flake updates are disabled in the
new repository until explicitly configured; see `VALIDATION.md`.
